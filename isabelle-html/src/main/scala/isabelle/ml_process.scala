...
//      }
      Nil // TODO try to load heaps?

    println(s"ml_process: heaps = $heaps")

    val eval_init: List[String] =
      if (heaps.isEmpty) {
        List(
          """
    ...
    println(s"ml_process: eval_init = $eval_init")

    val eval_modes: List[String] =
      if (modes.isEmpty) Nil
      else List("Print_Mode.add_modes " + ML_Syntax.print_list(ML_Syntax.print_string_bytes)(modes))

    println(s"ml_process: eval_modes = $eval_modes")
    ...
    // session base
    val eval_session_base: List[String] =
      session_base match {
        case None => Nil
        case Some(base) =>
    ...
    println(s"ml_process: eval_session_base = $eval_session_base")
    ...
    println(s"ml_process: eval_process = $eval_process")
    ...
    println(s"ml_process: ml_runtime_options = $ml_runtime_options")
    ...
    println(s"ml_process: bash_args = $bash_args")
