...
class Prover(
  ...
  private def system_output(text: String) {
    println(s"prover.system_output $text")
  ...
  private val process_manager = Standard_Thread.fork("process_manager") {
    ...
    val (startup_failed, startup_errors) = {
      ...
      println(s"prover: init process_manager loop done: ${finished.isEmpty} ${process.stderr.ready} ${process_result.is_finished}")
      (finished.isEmpty || !finished.get, result.toString.trim)
    }
    if (startup_errors != "") system_output(startup_errors)

    if (startup_failed) {
      println(s"prover: init process_manager failed: $startup_errors")
      terminate_process()
      process_result.join
      val res = process_result.get_finished
      println(s"prover: init process_manager failed, res = $res")
      exit_message(Process_Result(127))
    }
    else {
      println("prover: init process_manager success 1")
      val (command_stream, message_stream) = channel.rendezvous()

      command_input_init(command_stream)
      println("prover: init process_manager success 2")
      ...
      println("prover: init process_manager success 3")
      system_output("process terminated")
      command_input_close()
      for (thread <- List(stdout, stderr, message)) thread.join
      system_output("process_manager terminated")
      println("prover: init process_manager success 4")
      exit_message(result)
    }
    println("prover: init process_manager success END")
    channel.shutdown()
  }
  ...
  private def command_input_init(raw_stream: OutputStream) {
    println("prover.command_input_init")
          ...
          consume = {
            case chunks =>
              try {
                ...
                case e: IOException => system_output(name + ": " + e.getMessage); false
              }
          },
          finish = {
            case () => stream.close; system_output(name + " terminated")
          }
        )
      )
  }
  ...
  private def physical_output(err: Boolean): Thread = {
    ...
    Standard_Thread.fork(name) {
      try {
        ...
        case e: IOException => system_output(name + ": " + e.getMessage)
      }
