...
object Isabelle_Tool
{
  ...
  abstract class Body extends (List[String] => Unit)
...
Available tools:""" + tool_descriptions.mkString("\n  ", "\n  ", "\n")).usage
        case tool_name :: tool_args =>
          find_external(tool_name) match {
            case Some(tool) =>
              println(s"found external tool:")
              tool(tool_args)
            case None =>
              find_internal(tool_name) match {
                case Some(tool) =>
                  println(s"found internal tool:")
                  tool(tool_args)
                case None =>
                  error("Unknown Isabelle tool: " + quote(tool_name))
              }
          }
      }
    }
