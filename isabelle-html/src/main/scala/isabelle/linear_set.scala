...
object Linear_Set
{
  def fromSet(new_commands: Set[Command]): Linear_Set[Command] = {
    var res: Linear_Set[Command] = empty
    for (e <- new_commands) {
      res = res.incl(e)
    }
    res
  ...
  def empty[A] = empty_val.asInstanceOf[Linear_Set[A]]
  ...
  extends scala.collection.immutable.Set[A]
{
  ...
  override def incl (elem: A): Linear_Set[A] = insert_after(end, elem)

  override def excl (elem: A): Linear_Set[A] = delete_after(prev(elem))
